...
void serial_ternary(FSTR_P const pre, const bool onoff, 
FSTR_P const on, FSTR_P const off, FSTR_P const post/*=nullptr*/) 
{
...
void _print_xyz(NUM_AXIS_ARGS_(const float) FSTR_P const prefix) 
{
  ...
  #if NUM_AXES
    SERIAL_ECHOPGM_P(NUM_AXIS_PAIRED_LIST(SP_X_STR, x, SP_Y_STR, y, SP_Z_STR, z, 
      SP_I_STR, i, SP_J_STR, j, SP_K_STR, k,
      SP_U_STR, u, SP_V_STR, v, SP_W_STR, w));
  #endif
}

void print_xyz(NUM_AXIS_ARGS_(const float) 
FSTR_P const prefix/*=nullptr*/, FSTR_P const suffix/*=nullptr*/) 
{
